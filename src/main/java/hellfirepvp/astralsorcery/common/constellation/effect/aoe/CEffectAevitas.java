...
public class CEffectAevitas extends CEffectAbstractList<CropHelper.GrowablePlant> {
    ...
    public boolean playEffect(World world, BlockPos pos, ConstellationEffectProperties properties, @Nullable IMinorConstellation trait) {
        ...
        for (LivingEntity entity : entities) {
            if (entity.isAlive()) {
                if (properties.isCorrupted()) {
                    EntityUtils.applyPotionEffectAtHalf(entity, new EffectInstance(EffectsAS.EFFECT_BLEED, 200, amplifier * 2));
                    EntityUtils.applyPotionEffectAtHalf(entity, new EffectInstance(Effects.WEAKNESS, 200, amplifier * 3));
                    EntityUtils.applyPotionEffectAtHalf(entity, new EffectInstance(Effects.HUNGER, 200, amplifier * 4));
                    EntityUtils.applyPotionEffectAtHalf(entity, new EffectInstance(Effects.MINING_FATIGUE, 200, amplifier * 2));
                } else {
                    EntityUtils.applyPotionEffectAtHalf(entity, new EffectInstance(Effects.REGENERATION, 200, amplifier));
                }
            }
        }
