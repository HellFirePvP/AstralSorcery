...
public class AttributeLifeLeech extends PerkAttributeType {
    ...
    private static final float BASE_LEECH = 0F;
    ...
    public void onAttack(LivingDamageEvent event) {
        ...
        if (source.getTrueSource() != null && source.getTrueSource() instanceof EntityPlayer) {
            ...
            if (side == Side.SERVER && hasTypeApplied(player, side)) {
                float leechPerc = BASE_LEECH;
                leechPerc = PerkAttributeHelper.getOrCreateMap(player, side)
                        .modifyValue(AttributeTypeRegistry.ATTR_TYPE_ATTACK_LIFE_LEECH, leechPerc);
                leechPerc *= PerkAttributeHelper.getOrCreateMap(player, side).getModifier(AttributeTypeRegistry.ATTR_TYPE_INC_PERK_EFFECT);
                if (leechPerc > 0) {
